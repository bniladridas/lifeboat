...
def test_feature_engineering_family_size(mock_load, mock_artifacts):
    ...
    result = predict(
        pclass=1, sex="female", age=35, sibsp=1, parch=1, fare=100, embarked="C"
    )
...
def test_age_binning(mock_load, mock_artifacts):
    ...
    for age, expected_bin in test_cases:
        result = predict(
            pclass=3, sex="male", age=age, sibsp=0, parch=0, fare=10, embarked="S"
        )
...
def test_fare_per_person(mock_load, mock_artifacts):
    ...
    result = predict(
        pclass=3, sex="male", age=30, sibsp=2, parch=2, fare=40, embarked="S"
    )
...
def test_invalid_sex_value(mock_load, mock_artifacts):
    ...
    with pytest.raises(ValueError):
        predict(
            pclass=1, sex="unknown", age=25, sibsp=0, parch=0, fare=100, embarked="C"
        )
